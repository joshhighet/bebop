...
def getwallet_data(wallet, chain='btc'):
    ...
    if data.status_code == 200:
        logging.info('found %s wallet with balance %s (%s)', chain, data.json()['final_balance'], wallet)
        return data.json()
    logging.error('%s: failed to retrieve data on wallet - possibly a false match', wallet)
    return None
...
def main(sitesource):
    ...
    for currency, values in findings.items():
        ...
        for value in findings[currency]:
            logging.debug('found a potential %s wallet: %s', currency, value)
            if currency == 'btc':
                getwallet_data(value)
            elif currency == 'eth':
                getwallet_data(value, 'eth')
            else:
                logging.info('found suspected %s wallet: %s', currency, value)
    return findings
